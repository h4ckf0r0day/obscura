...
impl<C: OpMappingContext> OpDriver<C> for FuturesUnorderedDriver<C> {
  ...
  fn shutdown(&self) {
    ...
    self.queue.queue.pending.borrow_mut().clear();
    self.queue.queue.has_pending.set(false);
  }
...
impl<F: SubmissionQueueFutures> Queue<F> {
  fn spawn(&self, future: F::Future) {
    if self.polling.get() {
      self.pending.borrow_mut().push(future);
      self.has_pending.set(true);
    } else {
      self.queue.borrow_mut().spawn(future);
    }
    self.item_waker.wake_by_ref();
  }

  fn flush_pending(&self) {
    if !self.has_pending.replace(false) {
      return;
    }
...
struct PollingGuard<'a>(&'a Cell<bool>);
...
impl<F: SubmissionQueueFutures> SubmissionQueueResults<F> {
  pub fn poll_next_unpin(&mut self, cx: &mut Context) -> Poll<F::Output> {
    // A previous poll may have unwound after submitting nested work. Recover
    // that work before borrowing the main queue again.
    ...
    self.queue.polling.set(true);
    let polling = PollingGuard(&self.queue.polling);
    let result = queue.poll_next_unpin(cx);
    drop(polling);
    drop(queue);
    self.queue.flush_pending();
    result
