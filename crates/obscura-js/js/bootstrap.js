...
// Both stores live in Rust so they survive realm teardown. localStorage is
// shared by every page of one BrowserContext; sessionStorage is owned by the
// Page and keyed by origin, so it crosses same-origin navigation but never
// leaves the tab (issue #678).
const _storageOp = (slot) => slot.local ? Deno.core.ops.op_local_storage : Deno.core.ops.op_session_storage;
const _storageSnapshot = (slot) => {
  try { return JSON.parse(_storageOp(slot)('snapshot', '', '')); }
  catch (_) { return []; }
};
Storage.prototype.getItem = function(k) {
  const slot = _storageSlot(this);
  k = String(k);
  try { return JSON.parse(_storageOp(slot)('get', k, '')); }
  catch (_) { return null; }
};
Storage.prototype.setItem = function(k, v) {
  ...
  let stored = false;
  try { stored = JSON.parse(_storageOp(slot)('set', k, v)); }
  catch (_) {}
...
Storage.prototype.removeItem = function(k) {
  const slot = _storageSlot(this);
  k = String(k);
  _storageOp(slot)('remove', k, '');
};
Storage.prototype.clear = function() {
  const slot = _storageSlot(this);
  _storageOp(slot)('clear', '', '');
};
