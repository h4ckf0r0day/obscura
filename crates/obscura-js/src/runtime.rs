...
impl ObscuraJsRuntime {
    ...
    pub fn execute_script_guarded(&mut self, _name: &str, source: &str) -> Result<(), String> {
        ...
        // browser forever. HH login exposed that small scripts were executed
        // without the watchdog used for larger scripts, so a compact busy loop
        // could block full-load navigation before later defer/async scripts ran.
