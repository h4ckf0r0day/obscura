...
pub fn compute_render_warmup_styles(
    ...
    // A whole-document cascade pass is only needed when a stylesheet rule or
    // an inline style can actually produce a resource URL. A page whose CSS
    // contains no `url(...)` at all (the common case, and the shape of the
    // huge-DOM timing regression test) must not pay for building a style map.
    if !sheet.may_have_resource_urls()
        && shadow_sheets.values().all(|s| !s.may_have_resource_urls())
        && !dom_has_inline_style_url(tree)
    {
        return HashMap::new();
    }
...
#[cfg(feature = "paint")]
fn dom_has_inline_style_url(tree: &DomTree) -> bool {
    for nid in tree.descendants(tree.document()) {
        if let Some(node) = tree.get_node(nid) {
            if let Some(style) = node.get_attribute("style") {
                if style.as_bytes().windows(4).any(|w| w.eq_ignore_ascii_case(b"url(")) {
                    return true;
                }
            }
        }
