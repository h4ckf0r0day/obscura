            ...
            let resolution = match method {
                ...
                "Fetch.fulfillRequest" => {
                    ...
                    let headers = req.params.get("responseHeaders")
                        ...
                        .unwrap_or_default();
                    // `body` is a lossy text view; `body_base64` carries the CDP body
                    // (already base64) through unchanged so op_fetch_url can hand JS
                    // the exact bytes for a binary fulfill (#912). Decoding it here is
                    // also what checks it: a body that is not base64 reaches JS as the
                    // raw string either way, so it is refused at the boundary instead.
                    match decode_base64_bytes(raw_body) {
                        Ok(bytes) => Ok(obscura_js::ops::InterceptResolution::Fulfill {
                            status,
                            headers,
                            body: String::from_utf8_lossy(&bytes).to_string(),
                            body_base64: raw_body.to_string(),
                        }),
                        Err(why) => Err(format!("fulfillRequest body is not valid base64: {why}")),
                    }
                }
                "Fetch.failRequest" => {
...
/// Lossy text view of a base64 CDP field, for the two callers that serve it
/// as a string. `decode_base64_bytes` is the decoder; this only flattens its
/// result the way those call sites already expect.
pub(crate) fn decode_base64(input: &str) -> String {
    String::from_utf8_lossy(&decode_base64_bytes(input).unwrap_or_default()).to_string()
}
